...
import { MessageCircle, X, Send, Sparkles, MessageSquare, CheckCircle2, ThumbsUp, ThumbsDown, Zap, Award, Flame, BrainCircuit } from 'lucide-react';
...
import SemanticEngine from '../utils/SemanticEngine';
...
const MarketingBot: React.FC = () => {
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      // Bot Engine processes with hybrid semantic/rule logic
      const result = await getResponse(textToSend, recentHistory, userProfile, lastBotIntent, pendingAction);
      ...
      // Visual Highlighting based on intent keywords or semantic match
      const isHighIntent = 
      ...
      setIsTyping(false);
      console.error("Bot inference failed", err);
    }
  };
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-[1000] w-[380px] max-w-[calc(100vw-3rem)] h-[600px] bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.3)] border border-slate-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
          ...
          <div className="p-6 border-t border-slate-100 bg-white">
            {isBrainLoading ? (
              <div className="flex items-center justify-center gap-2 mb-4 p-3 bg-indigo-50 border border-indigo-100 rounded-2xl animate-pulse">
                <BrainCircuit size={16} className="text-indigo-600" />
                <span className="text-[9px] font-black uppercase text-indigo-700 tracking-widest">Initializing Local Brain (No Data Required)...</span>
              </div>
            ) : (
              <div className="flex items-center justify-center gap-2 mb-4">
                 <div className="w-1 h-1 bg-emerald-500 rounded-full" />
                 <span className="text-[8px] font-black uppercase text-slate-300 tracking-[0.2em]">Semantic Understanding Active</span>
              </div>
            )}
            
            <div className="relative">
              <input type="text" placeholder="Ask about price, security, or data..." className="w-full p-4 pr-12 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-emerald-500 font-medium transition-all" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} />
