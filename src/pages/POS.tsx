...
const POS: React.FC<POSProps> = ({ setView, currentUser, cart, setCart, parkTrigger }) => {
  ...
  const handleCheckoutComplete = async (sale: any, lowItems: string[]) => {
    ...
    if (lowItems.length > 0) {
      const now = Date.now();
      const lastSync = Number(localStorage.getItem('last_force_sync') || 0);
      const oneHour = 3600000;

      if (isSales && (now - lastSync > oneHour)) {
        setShowSyncRequired(true);
      } else {
        alert(`Sale completed! ⚠️ Note: ${lowItems.join(', ')} stock is finishing.`);
      }
    } else {
            ...
            {filteredProducts.map(product => (
              <button key={product.id} onClick={() => addToCart(product, 1)} className="group bg-white border border-slate-200 rounded-2xl p-4 text-left hover:border-emerald-500 hover:shadow-lg transition-all active:scale-95 flex flex-col h-full">
                ...
                <div className="flex items-center justify-between mt-auto">
                  ...
                  <span className={`text-[10px] font-bold uppercase ${product.stock_qty <= (product.low_stock_threshold || 5) ? 'text-rose-600' : 'text-slate-400'}`}>{product.stock_qty} left</span>
                </div>
              </button>
            ))}
          </div>
          ...
          {cart.length > 0 && (
            <div className="p-4 border-t space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={() => setShowCheckout(true)} className="py-4 bg-emerald-600 text-white rounded-2xl font-black text-sm shadow-xl shadow-emerald-200">Checkout</button>
              </div>
            </div>
          )}
        </div>
