...
const SupportChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      sender: 'bot',
      text: "👋 Welcome to NaijaShop! I am your Assistant Guru. I can check your stock, sales, and parked orders. Ask me anything!",
      timestamp: Date.now()
    }
  ]);
  ...
  // Keep live data for static template replacement, but use DB for dynamic queries
  ...
  // Calculate Real-time Shop Data for Template Variables
  ...
  const handleSend = async () => {
    ...
    try {
      // Process with async engine
      const match = await getBestMatch(userText, shopStats);
      ...
      const action = match?.action || 'NONE';
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        sender: 'bot',
        text: "I encountered a small network glitch. Please try asking again.",
        timestamp: Date.now()
      }]);
    }
  };
        ...
        <div className="fixed bottom-24 right-6 z-[999] w-[380px] max-w-[calc(100vw-3rem)] h-[600px] max-h-[calc(100vh-10rem)] bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.3)] border border-slate-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <div className="p-6 border-t border-slate-100 bg-white shrink-0">
            <div className="relative">
              <input 
                type="text"
                placeholder="Ask: 'How much Milo sold today?'"
