...
import { SyncProvider } from './hooks/context/SyncProvider';
    ...
    return (
      <div className="min-h-screen bg-emerald-950 flex items-center justify-center p-6">
        <div className="bg-white p-12 rounded-[3.5rem] text-center space-y-8 max-w-md w-full shadow-2xl">
          ...
          <button onClick={() => setCurrentView('settings')} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black text-xl"><CreditCard className="inline mr-2"/> Subscribe Now (₦10k/Yr)</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <SyncProvider currentUser={currentUser}>
      <Layout 
        ...
        onSubscribe={() => setCurrentView('settings')}
      >
        {currentUser ? (
          <>
            {currentView === 'dashboard' && <Dashboard currentUser={currentUser} setView={setCurrentView} isStaffLock={isStaffLock} trialRemaining={licenseInfo} isSubscribed={settings?.isSubscribed} />}
            ...
            {currentView === 'settings' && <Settings currentUser={currentUser} onSubscribe={() => {}} />}
              ...
              <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-200">
                <form onSubmit={handleLoginSubmit} className="space-y-6">
                  <div className="space-y-2">
                    <label className="block text-[10px] font-black text-slate-400 uppercase ml-1">Account</label>
                    <select required className="w-full px-5 py-4 bg-slate-50 border rounded-2xl font-bold outline-none" value={selectedStaffId} onChange={(e) => setSelectedStaffId(Number(e.target.value))}>
                      <option value="">Select Account</option>
                      {staffList.map(s => <option key={s.id} value={s.id!}>{s.name} ({s.role})</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="block text-[10px] font-black text-slate-400 uppercase ml-1">Access PIN</label>
                    <input required type="password" inputMode="numeric" placeholder="••••" className="w-full px-5 py-4 bg-slate-50 border rounded-2xl font-black text-center text-3xl tracking-[0.5em] outline-none" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value.replace(/\D/g, ''))} />
                  </div>
                  <button type="submit" className="w-full py-5 bg-emerald-600 text-white rounded-[2rem] font-black text-xl shadow-xl hover:bg-emerald-700 transition-all">Unlock Terminal</button>
                </form>
              </div>
            </div>
          </div>
