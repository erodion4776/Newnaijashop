...
// Add missing interface for InventoryProps
...
const Inventory: React.FC<InventoryProps> = ({ setView, currentUser, isStaffLock = false }) => {
  ...
  // Logic: Reconcile all products on load to detect discrepancies
  useEffect(() => {
    if (allProducts.length > 0) {
      const runAudit = async () => {
        ...
        for (const p of allProducts) {
          if (p.id) {
            const audit = await reconcileStock(p.id);
            report[p.id] = audit.match;
          }
        }
  ...
  const generateInventoryPDF = () => {
    const doc = new jsPDF();
    const shopName = settings?.shop_name || 'NaijaShop POS';
    ...
    doc.text('Official Inventory Report', 14, 32);
    ...
    autoTable(doc, {
      startY: 45,
      head: [['S/N', 'Product Name', 'Category', 'Stock Qty', 'Selling Price']],
      body: tableData,
      headStyles: { 
        fillColor: emeraldRGB as any, 
        textColor: 255, 
        fontStyle: 'bold' 
      },
      columnStyles: {
    ...
    doc.setTextColor(0);
    doc.setFont('helvetica', 'bold');
    doc.text(`Total Unique Items: ${allProducts.length}`, 14, finalY + 12);
    
    doc.setFontSize(8);
    doc.setTextColor(180);
    doc.setFont('helvetica', 'italic');
    doc.text('Generated by NaijaShop POS - Professional SME Terminal', 14, doc.internal.pageSize.height - 10);

    const fileDate = new Date().toISOString().split('T')[0];
    const fileName = `${shopName.replace(/\s+/g, '_')}_Inventory_${fileDate}.pdf`;
    doc.save(fileName);
  };
  ...
  /**
   * ATOMIC: handleSubmit with Transaction
   */
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Fix: Cast db to any to resolve 'Property transaction does not exist on type NaijaShopDB'
      await (db as any).transaction('rw', [db.products, db.inventory_logs, db.audit_trail], async () => {
        if (editingProduct?.id) {
          const oldProduct = await db.products.get(editingProduct.id);
          const oldStock = Number(oldProduct?.stock_qty || 0);
          ...
          if (oldStock !== newStock) {
            ...
            // NOTE: Hook in db.ts will automatically handle inventory_logs
          }
        } else {
          ...
          // Manual log for "Initial Stock" as it's a creation, not an update (hook only hits updates)
  ...
  /**
   * ATOMIC: BulkUpdate with Transaction
   */
  const handleBulkUpdate = async () => {
    ...
    try {
      // Fix: Cast db to any to resolve 'Property transaction does not exist on type NaijaShopDB'
      await (db as any).transaction('rw', [db.products, db.audit_trail], async () => {
        const updatedProducts = targetProducts.map(p => {
          let adjustment = 0;
          if (bulkType === 'percent') {
            adjustment = Number(p.price) * (Number(bulkValue) / 100);
          } else {
            adjustment = Number(bulkValue);
          }

          let newPrice = bulkDirection === 'increase' ? Number(p.price) + adjustment : Number(p.price) - adjustment;
          // Naija Rounding to nearest 50
  ...
  /**
   * ATOMIC: Restock with Transaction
   */
  const handleRestockSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await (db as any).transaction('rw', [db.products, db.inventory_logs, db.audit_trail], async () => {
        ...
        // NOTE: The update hook in db.ts will capture this change and create the inventory_log
      });
    ...
    try {
      ...
      await (db as any).transaction('rw', [db.products, db.inventory_logs], async () => {
        const productsToAdd = products.map(p => ({
          ...p,
          cost_price: Math.round((Number(p.price) * 0.8) / 50) * 50, // Default 20% margin
          category: 'General',
          low_stock_threshold: 5
        }));
        
        const ids = await db.products.bulkAdd(productsToAdd as Product[], { allKeys: true });
        
        // Logs for new items
        const logs = productsToAdd.map((p, idx) => ({
          product_id: ids[idx],
          product_name: p.name,
          quantity_changed: Number(p.stock_qty),
          old_stock: 0,
          new_stock: Number(p.stock_qty),
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          >
            <FileText size={18} /> Download Price List
          </button>
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-1">
                      {!isAudited && (
                        <div className="flex items-center gap-1 text-[9px] font-black px-2.5 py-1 bg-amber-500 text-white rounded-full uppercase tracking-widest" title="Stock Discrepancy Detected">
                          <ShieldAlert size={10} /> Audit Required
                        </div>
                      )}
                      {reserved > 0 && (
      ...
      {isDeleteModalOpen && deleteProduct && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
           <div className="bg-white p-10 rounded-[3rem] text-center space-y-8 w-full max-w-sm animate-in zoom-in">
              <div className="w-20 h-20 bg-rose-50 text-rose-600 rounded-full flex items-center justify-center mx-auto shadow-inner"><Trash size={40} /></div>
